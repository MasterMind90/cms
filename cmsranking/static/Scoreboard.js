    ...
    <th class=\"team\">Team</th>";
        ...
        for (var i in contests) {
            ...
            for (var j in tasks) {
                var task = tasks[j];
                var t_id = task["key"];
                var task_name = task["short_name"];
                if (Config.show_full_task_name) {
                    task_name = escapeHTML(task["name"]);
                }

                result += " \
    <th colspan=\"3\" class=\"score task\" data-task=\"" + t_id + "\" data-sort_key=\"t_" + t_id + "\"><abbr title=\"" + escapeHTML(task["name"]) + "\">" + task_name + "</abbr></th>";
            }
